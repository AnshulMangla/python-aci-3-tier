...
def main():
    ...
    # yml_env_file = open("./vars/public_sandbox.yml").read()
    yml_env_file = open("./vars/blre_env.yml").read()
    yml_env_dict = yaml.load(yml_env_file, yaml.SafeLoader)

    url = yml_env_dict['url']

    user = yml_env_dict['user']
    pwd = yml_env_dict['pwd']

    # Initiate the session with APIC
